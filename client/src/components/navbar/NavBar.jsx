...
const NavBar = () => {
  ...
  return(
    <div className='navbar'>
      <div className='list'>
      {isOpen && <CloseRoundedIcon className='icon' onClick={handleHideMenu}/>}
      {isOpen && <MenuBar titles={titles}/>}
      {!isOpen && responsive && <MenuRoundedIcon className='icon' onClick={handleMenu}/>}
      {isOpen && <div className="hideDiv" onClick={handleHideMenu}></div>}
      { !responsive && <NavLink to="" className="tb"  exact>Tableau de bord</NavLink>}
      { !responsive &&<ul className='sublist'> 
          {titles.map((title,i) => <li key={title.id}><NavLink to={title.path} className="link"  exact>{title.title}</NavLink></li>)}
        </ul>}
       </div>
